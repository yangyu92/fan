...
class FanWidget extends StatefulWidget {
  ...
  const FanWidget({Key? key, required this.onGradeSelect}) : super(key: key);
...
class FanBtnPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < manage.list.length; i++) {
      ...
      canvas.save();
      // canvas.translate(
      //     -(manage.list.length * model.size) / 2 +
      //         model.size * i +
      //         model.size / 2,
      //     0);
    ...
    switch (type) {
      ...
      case FanGradeType.grade1:
        gradePath = Path()
          ..moveTo(-size / 2 + 10, 5)
          ..lineTo(size / 2 - 20, 5)
          ..relativeQuadraticBezierTo(10, 0, 10, -8)
          ..relativeQuadraticBezierTo(-5, -10, -10, 0);
        break;
      case FanGradeType.grade2:
        gradePath = Path()
          ...
          ..relativeQuadraticBezierTo(-5, 6, -10, 0);
        break;
