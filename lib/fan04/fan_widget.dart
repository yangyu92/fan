...
class _FanWidgetState extends State<FanWidget>
  ...
  void startUpdateMax(double max) {
    pm.updateMax(max);
    if (max == 0) upholderNum = 0;
    if (max == 6) upholderNum = 1;
    if (max == 8) upholderNum = 2;
    if (max == 10) upholderNum = 3;
    setState(() {});
...
class UpholderPainter extends CustomPainter {
  final int upholderNum;

  UpholderPainter({this.upholderNum = 0});
  ...
  void _drawLamp(Canvas canvas, Size size) {
    ...
    canvas.translate(0, -(height - lampHeight * upholderNum) / 2 + 10);
    Path lampPath = Path()
      ..addRRect(
        RRect.fromRectAndRadius(
          Rect.fromCenter(
